...
def exec_code(orig, code, stream):
    ...
    if lang == "python" or lang == "py":
        ...
        with open('./comp_code/code.c', 'w') as fp:
            fp.write(code)
        val = subprocess.run('g++ -o ./comp_code/res_c ./comp_code/code.c; ./comp_code/res_c', shell=True,
                             capture_output=True)
        if val.stderr:
        ...
        with open('./comp_code/code.cpp', 'w') as fp:
            fp.write(code)
        val = subprocess.run('g++ -o ./comp_code/res_cpp ./comp_code/code.cpp; ./comp_code/res_cpp', shell=True,
                             capture_output=True)
        if val.stderr:
            print(val.stderr.decode("utf-8"))
        elif val.stdout:
...
async def naujieji():
    date = datetime.now()
    print(f"Laukiama naujųjų: {date}")

    if date.hour != 0 or date.minute != 0 or date.year != 2022:
        return
    ...
    if not channel:
        ...
        try:
            await channel.send(content="https://media0.giphy.com/media/26tOZ42Mg6pbTUPHW/giphy.gif")
            await channel.send(content="https://i.pinimg.com/originals/1b/4c/27/1b4c27fd5b5f69c78172e1ce5f509329.gif")
            await channel.send(content="https://i.pinimg.com/originals/9e/bf/a7/9ebfa7a509fb8a748c61497ac95a0ac3.gif")
            await channel.send(content="https://i.pinimg.com/originals/45/14/00/451400f7046c8c51536e3249b973cd3f.gif")

            allowed_mentions = discord.AllowedMentions(everyone=True)

            await channel.send(content="@everyone Su naujaisiais!!!", allowed_mentions=allowed_mentions)
        except discord.Forbidden as e:
            print(e, file=sys.stderr)
